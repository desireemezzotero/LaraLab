...
@section('component')
    <div class="container mx-auto pb-10">
        <h1 class="text-2xl font-bold text-gray-800 text-center mb-6">Modifica Pubblicazione: {{ $publication->title }}</h1>
                        ...
                        @foreach ($users as $user)
                            ...
                            <div class="flex items-center justify-between bg-white p-2 rounded border shadow-sm">
                                ...
                                <input type="number" name="positions[{{ $user->id }}]" value="{{ $position }}"
                                    min="1" class="w-14 text-xs border-gray-200 rounded p-1 text-center">
                            </div>
                        @endforeach
                    </div>
                </div>
