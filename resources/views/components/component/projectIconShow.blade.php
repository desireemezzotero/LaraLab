...
@if ($user->role === 'Admin/PI' || $isManager)
    {{-- Icona Modifica --}}
    <a href="{{ route('project.edit', $project->id) }}" class="text-emerald-700 transition transform hover:scale-150"
...
@if ($user->role === 'Admin/PI')
    <form action="{{ route('project.destroy', $project->id) }}" method="POST"
        ...
        @csrf
        @method('DELETE')
        <button type="submit" class="text-red-500 transition transform hover:scale-150" title="Elimina Progetto">
